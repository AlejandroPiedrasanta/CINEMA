...
- 2026-07-02: **Sistema de Actualizaciones vía Base de Datos**: Al descargar el ZIP desde Ajustes → se registra automáticamente en MongoDB con versión timestamp. Todas las Apps de Escritorio que comparten la misma base de datos detectan la nueva versión directamente consultando MongoDB (sin llamadas HTTP). `version.txt` embebido en el ZIP = identificador de versión local. Banner automático al abrir la app.
